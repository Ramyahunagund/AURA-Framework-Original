...
def pause_plan_on_banner_and_plan_details_card_on_sub_with_paused_plan_on_overview_page(stage_callback):
    framework_logger.info("=== C43768665 - Pause plan on banner and Plan details card on sub with paused plan on Overview Page flow started ===")
    ...
    with PlaywrightManager() as page:
        try:
            ...
            # Access Dashboard
            DashboardHelper.first_access(page)
            framework_logger.info(f"Accessed Dashboard")
            ...
            # Sees Plan pause on plan details card on Overview page
            expect(overview_page.pause_plan_info).to_be_visible(timeout=90000)
            framework_logger.info(f"Verified Plan Pause information on plan details card on Overview page")

            # Click on Resume Plan link on plan details card on Overview page
            overview_page.resume_plan_link.click()
            framework_logger.info(f"Clicked on Resume Plan link on plan details card on Overview page")
            ...
            # Sees original plan on Overview page
            expect(overview_page.plan_100_resume_text).to_be_visible()
            expect(overview_page.plan_100_resume_text).to_contain_text("Your 100 pages per month")
            framework_logger.info(f"Verified original plan on Overview page")

            # Click on Resume
            overview_page.resume_plan_link.click()
            framework_logger.info(f"Clicked on Resume button on Plan Card on Overview page")
            ...
            framework_logger.info("=== C43768665 - Pause plan on banner and Plan details card on sub with paused plan on Overview Page flow finished ===")
