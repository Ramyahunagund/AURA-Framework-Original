...
def subscribed_and_redeemed_paper_subscription_update_plan_page(stage_callback):
    framework_logger.info("=== C31446176 / C31446178 - Subscribed and Redeemed Paper Subscription Update Plan Page flow started ===")
    common.setup()
    ...
    with PlaywrightManager() as page:
        ...
        try:
            ...
            # Verify free month value
            DashboardHelper.verify_free_months_value(page, 2)
            framework_logger.info("Verified free month value on Overview page")

            # Access Update Plan page
            side_menu.click_update_plan()
            framework_logger.info("Navigated to Update Plan page")

            # Verify available plans
            UpdatePlanHelper.verify_available_plans(page, plan_type="ink_and_paper")
            framework_logger.info("Verified available plans on Update Plan page")

            # Verify header links
            expect(update_plan.header_cancel_link).to_be_visible()
            expect(update_plan.header_remove_paper_link).to_be_visible()
            framework_logger.info("Verified header links are visible on Update Plan page")

            # Verify paper free month disclaimer
            expect(update_plan.paper_free_months).to_be_visible()
            framework_logger.info("Verified paper free month disclaimer is visible on Update Plan page")

            # Verify links on plan details card
            expect(update_plan.cancel_instant_ink).to_be_visible()
            expect(update_plan.cancel_paper_add_on).to_be_visible()
            expect(update_plan.replace_printer_link).to_be_visible()
            expect(update_plan.contact_support_link).to_be_visible()
            expect(update_plan.prepaid_code_link).to_be_visible()
            framework_logger.info("Verified links are visible on Update Plan page")  

            # Verify free month value
            DashboardHelper.verify_free_months_value(page, 2)    
            framework_logger.info("Verified free month value on Update Plan page")      

            framework_logger.info("=== C31446176 / C31446178 - Subscribed and Redeemed Paper Subscription Update Plan Page flow finished successfully ===")
        except Exception as e:
