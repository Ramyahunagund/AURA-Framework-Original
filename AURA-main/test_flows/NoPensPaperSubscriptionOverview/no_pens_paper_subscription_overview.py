...
def no_pens_paper_subscription_overview(stage_callback):
    framework_logger.info("=== C31739055 - No pens paper subscription overview flow started ===")
    common.setup()
    ...
    with PlaywrightManager() as page:
        ...
        try:
            # Verify the subscription state is subscribed_no_pens
            GeminiRAHelper.access(page)
            GeminiRAHelper.access_tenant_page(page, tenant_email)
            GeminiRAHelper.access_subscription_by_tenant(page)
            GeminiRAHelper.verify_rails_admin_info(page, "State", "subscribed_no_pens")
            framework_logger.info("Verified subscription state is subscribed_no_pens")
            
            # Access Dashboard
            DashboardHelper.first_access(page, tenant_email)
            framework_logger.info("Opened Instant Ink Dashboard")

            # Monthly Summary section and Monthly Summary Paper section are displayed
            expect(overview_page.monthly_summary).to_be_visible(timeout=90000)
            expect(overview_page.paper_monthly_summary).to_be_visible()
            framework_logger.info("Verified Monthly Summary sections are displayed")

            # Verify message
            expect(overview_page.paper_no_pens_message).to_be_visible()
            framework_logger.info("Verified no pens paper message is displayed")
            ...
            expect(overview_page.paper_status).to_contain_text("We’ll send your first paper shipment after you install your HP Instant Ink cartridges.")
            framework_logger.info("Verified Paper Status Card is displayed")

            # Click on last shipment link
            overview_page.paper_last_shipment.click()
            expect(shipment_tracking_page.page_title).to_be_visible(timeout=90000)
            framework_logger.info("Verified Shipment Tracking page is opened from last shipment link")

            framework_logger.info("=== C31739055 - No pens paper subscription overview flow finished successfully ===")
        except Exception as e:
