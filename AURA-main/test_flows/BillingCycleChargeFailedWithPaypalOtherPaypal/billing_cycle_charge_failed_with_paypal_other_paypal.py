...
def billing_cycle_charge_failed_with_paypal_other_paypal(stage_callback):
    framework_logger.info("=== C49180267 - Billing Cycle Charge Failed with PayPal payment - Other PayPal started ===")
    ...
    with PlaywrightManager() as page:
        ...
        try:
            ...
            # Rails admin user edit the Pgs Paypal credential
            GeminiRAHelper.access_tenant_page(page, tenant_email)
            GeminiRAHelper.pgs_paypal_credential_edit(page, '1')
            framework_logger.info("PGS PayPal credential edited")
            ...
            DashboardHelper.add_paypal_billing(page, "paypal_2")
            framework_logger.info(f"Added Paypal as payment method")

            # Set the original information on Pgs Paypal credential and user updates Paypal override field to settled
            GeminiRAHelper.access(page)
            GeminiRAHelper.access_tenant_page(page, tenant_email)
            GeminiRAHelper.pgs_paypal_credential_edit(page, 'token')
            framework_logger.info("PGS Paypal credential edited")
            ...
            framework_logger.info("=== C49180267 - Billing Cycle Charge Failed with PayPal payment - Other PayPal finished ===")
